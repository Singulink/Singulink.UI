...
public static class DialogPresenterExtensions
{
    ...
    /// <summary>
    /// Shows a message dialog with the specified options and returns a task with the resulting clicked button index when the dialog closes. The options allow
    /// some additional customization of the dialog, such as configuring the default and cancel buttons.
    /// </summary>
    /// <param name="navigator">The instance of the navigator to use to show the dialog.</param>
    ...
    public static async Task<int> ShowMessageDialogAsync(this IDialogPresenter navigator, MessageDialogOptions options)
    {
        ...
        return await navigator.ShowDialogAsync(viewModel);
    }
}
