    ...
    public void AddRoute(RouteBase route)
    {
        ...
        if (route.ParentViewModelType is not null)
        {
            if (!_routeList.Any(r => r.ViewModelType == route.ParentViewModelType))
                throw new InvalidOperationException("Route's parent view model type does not have any routes added yet. Add parent routes before child routes.");
        ...
        if (!_vmTypeToViewInfo.ContainsKey(route.ViewModelType))
            throw new InvalidOperationException("Route's view model type has not been mapped to a view.");
