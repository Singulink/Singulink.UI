...
- On WebAssembly, browser URLs are now relative to the site path the application is hosted under (`Navigator.BrowserBasePath`, from the Uno `WasmShellWebAppBasePath` property), so applications hosted under a sub-folder get correct deep links and address bar updates. See [Hosting Under a Sub-Folder](winui-setup.md#hosting-under-a-sub-folder).
