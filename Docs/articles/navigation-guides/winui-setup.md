...
```xml
<PropertyGroup>
  <WasmShellWebAppBasePath>/app/</WasmShellWebAppBasePath>
</PropertyGroup>
```

With that in place a deep link to `https://example.com/app/r/my-repo/document/42` yields the route `/r/my-repo/document/42` from `GetBrowserRoute()`, and navigating to that route writes `/app/r/my-repo/document/42` to the address bar. Routes themselves never include the base path. The Uno.Sdk defaults the property to `/`, which is correct for an application hosted at the site root; a relative value such as `./` is resolved against the folder the page was loaded from.

The web server must also serve the application's `index.html` for any route under the base path (an SPA fallback rewrite), otherwise refreshing or sharing a deep link returns a 404. The `web.config` and `staticwebapp.config.json` files generated by the Uno templates do this.
